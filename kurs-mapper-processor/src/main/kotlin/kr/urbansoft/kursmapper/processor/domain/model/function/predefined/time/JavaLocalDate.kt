...
val Companion.JavaLocalDate: PreDefinedMappingFunctions
  get() = concrete

private val concrete =
  object : PreDefinedMappingFunctions {
    override fun Context.collect(): List<MappingFunction> = listOf(asLocalDate())

    private fun Context.asLocalDate(): MappingFunction =
      MappingFunction.createPreDefined(
        sourceId = KursTypeName.from("java.time.LocalDate").asKursTypeId(),
        targetId = KursTypeName.from("kotlinx.datetime.LocalDate").asKursTypeId(),
        body =
          FunctionBody.build(functionBodyBuilderContext()) {
            returnKeyword()
            source()
            dot().text("toKotlinLocalDate").invoke()
          },
        importList =
          FunctionBody.Import.buildList(functionBodyBuilderContext()) {
            add("kotlinx.datetime", "toKotlinLocalDate")
          },
      )
  }
